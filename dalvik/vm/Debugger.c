...
//-----------------------------------------------------------------------------
    if (dvmHasCARClassAnnotation(clazz)) {
    	for (iCARObjCount = 0, clz = clazz;  clz != NULL;  clz = clz->super) {
            if (IS_CLASS_FLAG_SET(clz, CLASS_CAR_CLASS|CLASS_CAR_INTERFACE))
                iCARObjCount++;
        }
        declared = clazz->sfieldCount + clazz->ifieldCount+iCARObjCount;
    } else {
...
//-----------------------------------------------------------------------------
    ...
    for (i = 0; i < clazz->sfieldCount; i++) {
	    char        *sGetter, *sSetter;
	    char        *sClassDescriptor;
        char         buf[512];
	    char         funcNameS[128], funcNameG[128];

        field = (Field*) &clazz->sfields[i];
		StaticField  *sfield = (StaticField*)field;

        if (dvmHasCARMemberAnnotation(field)) {
	        sClassDescriptor = dvmGetCARMemberAnnotationValue(field, "ClassDescriptor");
            if (sClassDescriptor == NULL || *sClassDescriptor == '\0')
                sClassDescriptor = "NULL";
	        sGetter = dvmGetCARMemberAnnotationValue(field, "Getter");
            if (sGetter == NULL || *sGetter == '\0') {
	            strcpy(funcNameG, "get");
	            strcat(funcNameG, field->name);
                sGetter = funcNameG;
            }

	        sSetter = dvmGetCARMemberAnnotationValue(field, "Setter");
            if (sSetter == NULL || *sSetter == '\0') {
	            strcpy(funcNameS, "set");
	            strcat(funcNameS, field->name);
                sSetter = funcNameS;
            }
            snprintf(buf, 512, "%s,sClassDescriptor=%s[0x%X],Getter=%s[0x%X],Setter=%s[0x%X]", \
                     field->name, sClassDescriptor, sfield->pSetterGetterClassObject, \
                     sGetter, sfield->getter, sSetter, sfield->setter);
        } else {
            strcpy(buf, field->name);
        }

        expandBufAddFieldId(pReply, fieldToFieldId(field));
        expandBufAddUtf8String(pReply, (const u1*) buf);
    ...
    for (i = 0; i < clazz->ifieldCount; i++) {
        ...
        if (dvmHasCARMemberAnnotation(field)) {
            ...
            snprintf(buf, 512, "%s,Getter=%s[0x%X],Setter=%s[0x%X]", field->name, \
                     sGetter, ifield->getter, sSetter, ifield->setter);
        } else {
            strcpy(buf, field->name);
        }

		fId = fieldToFieldId(field);

		if (field->signature[0]=='L')
		{
			ClassObject* fieldClazz = dvmFindClass(field->signature,clazz->classLoader);
			if (strcmp(fieldClazz->descriptor,"Ljava/lang/Callbacks;")==0 || \
			  ...
			  (fieldClazz->super!=NULL && strcmp(fieldClazz->super->descriptor,"Ljava/lang/Delegates;")==0)) {
				strcpy(buf, "DelegateObjDesc");
			}
		}

        expandBufAddFieldId(pReply, fId);
        expandBufAddUtf8String(pReply, (const u1*) buf);
        expandBufAddUtf8String(pReply, (const u1*) field->signature);
        if (withGeneric)
            expandBufAddUtf8String(pReply, genericSignature);
...
//-----------------------------------------------------------------------------
    if (dvmHasCARClassAnnotation(clazz)) {
    	for (i = 0, clz = clazz;  clz != NULL;  clz = clz->super) {
            if (IS_CLASS_FLAG_SET(clz, CLASS_CAR_CLASS|CLASS_CAR_INTERFACE)) {
                expandBufAddFieldId(pReply, MAX_PSEUDO_FIELD_ID-i);
                expandBufAddUtf8String(pReply, (const u1*)"CARObjDesc");
                expandBufAddUtf8String(pReply, (const u1*)"Lcom/kortide/CARObjDesc;");
                if (withGeneric)
                    expandBufAddUtf8String(pReply, genericSignature);
                expandBufAdd4BE(pReply, 0);    //accessFlags
                i++;
            }
        }
    }
