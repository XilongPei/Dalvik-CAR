...
tryArchive:
        ...
        if (entry != NULL) {
            ...
            if (newFile) {
                ...
                //extract CAR file in the JAR file into the cache directory
                ZipEntry entry2;
                int   fd2;
                int   hashEnt = 0;
                char *cachedName2;
                ...
                while (1) {
                    entry2 = dexZipFindCARModuleEntry(&archive, &hashEnt, buf);
                    if (entry2 != NULL) {
                        cachedName2 = dexOptGenerateCacheFileName(fileName, buf);
                        if (cachedName2 == NULL) {
                            LOGI("Unable to open or create cache for %s\n", fileName);
                            goto bail;
                        }

                        fd2 = open(cachedName2, O_CREAT|O_RDWR, 0644);
                        free(cachedName2);
                        if (fd2 < 0) {
                            LOGI("Unable to open or create cache for %s\n", fileName);
                            goto bail;
                        }
                        result = dexZipExtractEntryToFile(&archive, entry2, fd2);
                        close(fd2);
    ...
    *ppJarFile = (JarFile*) calloc(1, sizeof(JarFile));
    ...
    /* for CAR
     *
     * when load a Java class, it could ask to load a CAR module, the CAR module
     * should in the same path of file file of the Java class, normally.
     */
    pDvmDex->pDexFile->dexFilePath = cachedName;
