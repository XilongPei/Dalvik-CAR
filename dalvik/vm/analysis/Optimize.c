    ...
    OpCode volatileOpc)
{
    ...
    if (volatileOpc != OP_NOP && dvmIsVolatileField(&instField->field)) {
        ...
        /*
         *  get the annotation field information, if it has CAR setter/getter,
         *  don't optimize this instruction, for the CAR setter/getter depends on
         *  InstField information. so, the field must be resolved when running,
         *  couldn't be accessed by field->byteOffset
         */
        //use anontation information to do this, speed up the dex optimize
        //if (field->getter != NULL || field->setter != NULL)
        if (dvmHasCARMemberAnnotation(&(instField->field)))
        {
        	LOGD("CAR Member Annotation,%s", instField->field.name);
            return true;
        }

    	if (instField->field.signature[0] == 'L') {
            const DexClassDef  *pClassDef;
            u4            u4tmp;
    		DexFile      *pDexFile;

            pDexFile=instField->field.clazz->pDvmDex->pDexFile;

            pClassDef = dexFindClass(pDexFile, instField->field.signature);
            if (pClassDef == NULL)
                goto NORMAL;

            u4tmp = pDexFile->classIdxDelegates;
            if (pClassDef->classIdx == u4tmp || pClassDef->superclassIdx  == u4tmp ) {
        		LOGD("DelegateHandler,%s", instField->field.name);
            	return true;
    		} else {
                u4tmp = pDexFile->classIdxCallbacks;
    			if (pClassDef->classIdx == u4tmp || pClassDef->superclassIdx  == u4tmp ) {
            		LOGD("CallbackHandler,%s", instField->field.name);
            		return true;
            	}
        	}
    	}
