    ...
    JNIEnv* pEnv)
{
    ...
    dvmCheckAsmConstants();
   	...
   	//init Java&CAR interoperate environment
	if (!dvmElapiInitialize())
...
void dvmShutdown(void)
{
    ...
    dvmShutdownMsgLoopThread(gDvm.pAppletObj);
    /*uninitialize the JAVA&CAR interoperate environment, which could be cited
      in Java.finzlize, so it should be uninitialized lastly.
    */
    dvmElapiUninitialize();
