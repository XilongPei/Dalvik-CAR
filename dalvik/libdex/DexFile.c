...
DexFile* dexFileParse(const u1* data, size_t length, int flags)
{
    ...
    /*
     * for CAR, speed up the process about class "Ljava/lang/CallbackHandler;" and
     * "Ljava/lang/DelegateHandler;"
     */
    unsigned int idx;

    pDexFile->classIdxCallbacks = (u4)-2;
    pDexFile->classIdxDelegates = (u4)-2;
    for (idx = 0;  idx < pDexFile->pHeader->typeIdsSize && \
                       (pDexFile->classIdxCallbacks == (u4)-2 || pDexFile->classIdxDelegates == (u4)-2);
             idx++) {

        char *s = dexStringByTypeIdx(pDexFile, idx);

        //LOGD("types[idx=%d]%s", idx, s);

        if (pDexFile->classIdxCallbacks == (u4)-2 && strcmp(s, "Ljava/lang/Callbacks;") == 0 )
            pDexFile->classIdxCallbacks = idx;
        else if (pDexFile->classIdxDelegates == (u4)-2 && strcmp(s, "Ljava/lang/Delegates;") == 0 )
            pDexFile->classIdxDelegates = idx;
    }
