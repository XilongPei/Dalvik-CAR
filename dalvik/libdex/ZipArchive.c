...
/*
 * find CAR file, ".eco",  in a JAR file
 *
 * Returns 0 if not found.
 */
ZipEntry dexZipFindCARModuleEntry(const ZipArchive *pArchive, int *startEnt, char *nameBuf)
{
    const int hashTableSize = pArchive->mHashTableSize;
    int ent = *startEnt;
    int nameLen;
    ...
    for (ent = *startEnt;  ent < hashTableSize;  ent++) {
        s = (const char *)pArchive->mHashTable[ent].name;
        nameLen = pArchive->mHashTable[ent].nameLen;
