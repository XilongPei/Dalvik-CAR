...
//----------the raw version in Dalvik
#ifndef __JNI_SUPPORT_CAR__
...
//----------for CAR support
            case 'L': {     /* 'shorty' descr uses L for all refs, incl array */
                ...
                assert(obj == NULL || dvmIsValidObject(obj));
                if (fromJni) {
                    argObj = (u4) dvmDecodeIndirectRef(env, argObj);
                } else if (method->pCARMethodInfo != NULL && method->pCARMethodInfo->needCopy) {
                    //get the Object's [in] parameter, if it is not a primative datatype, we should convert it
                    int  i;
                    i = desc - &(method->shorty[0]);

                    argObj = dvmCARObject2JavaObject(self, obj, method, argObj, i);
                }
...
#ifdef __JNI_SUPPORT_CAR__
//----------for CAR support
    //convert pResult if nessary
    if (method->shorty[1] == 'L' && !fromJni) {
        if (method->pCARMethodInfo != NULL && method->pCARMethodInfo->needCopy)
            pResult->i = dvmJavaObject2CARObject(self, obj, method, pResult->i, 0);
    }
//----------
#endif
...
//----------
        case 'L': {
            if (fromJni) {
                ...
                *ins++ = dvmCARObject2JavaObject(self, obj, method, (u4)args->l, i);
            } else {
                *ins++ = (u4)args->l;           /* get all 32 bits */
            }
            break;
        }
