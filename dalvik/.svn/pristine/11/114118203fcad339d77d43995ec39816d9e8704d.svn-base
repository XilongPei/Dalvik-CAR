...
static int computeCARJniArgInfo(const DexProto* proto);
...
size_t dvmClassObjectSize(const ClassObject *clazz)
{
    ...
    size = classObjectSize(clazz->sfieldCount);

    for (;  clazz->super != NULL;  clazz = clazz->super) {
...
static ClassPathEntry* processClassPath(const char* pathStr, bool isBootstrap)
{
    ...
    while (cp < end) {
        if (*cp == '\0') {
            ...
            //for CAR component cache
            char *s = strrchr(cp, '.');
            if ((s != NULL) && (strcmp(s, ".eco") == 0)) {
                dvmCacheCARModuleInBootclasspath(cp);
                continue;
            }
    ...
    idx++;
    if (idx < count)
        gDvm.bootClassPath = realloc(cpe, idx*sizeof(ClassPathEntry));
...
#if 0
//we change the statements for Java&CAR, the original is after else statement

    if (found == (void*) clazz) {
        //check the CLASS_CAR_CLASS
        ClassObject* clzTmp;
        bool meetCAR_CLASS = false;
        bool meetCAR_INTERFACE = false;

        for (clzTmp = clazz;  clzTmp != NULL;  clzTmp = clzTmp->super) {
            if (IS_CLASS_FLAG_SET(clzTmp, CLASS_CAR_CLASS))
                meetCAR_CLASS = true;
            if (IS_CLASS_FLAG_SET(clzTmp, CLASS_CAR_INTERFACE))
                meetCAR_INTERFACE = true;
        }

        if (meetCAR_INTERFACE) {

        	  /* error statement, the object relation diagram

                          NULL(java.lang.Object)
                          /
                         /
                       Clazz ->CARInterface <--- should not appeares in inheritting chain
                       /
                      /
                    Clazz
                    /
            ...
            */
            ...
            dvmThrowExceptionWithClassMessage("Ljava/lang/InstantiationException;",clazz->descriptor);
        }

        if (meetCAR_CLASS) {
            //if the clazz's has already been set CLASS_CAR_CLASS, it is allowed
            SET_CLASS_FLAG(clazz, CLASS_CAR_CLASS);
        }
    ...
    const u1* pEncodedData, Object* classLoader)
{
    ...
    if (gDvm.shouldCARWorking) {
    	...
    	{
    		/* if this is a class with CARClass annotation, we reflect the CAR information here,
    		 * and new a PInterface for the Class, store in newclass->thisCAR. This PInterface
    		   is to destoryed as the newClass is destroyed.
    		*/
    		...
    		/* if this is a class with CARInterface annotation, we
    		 * set the flag here.
    		*/
    ...
    if (pHeader->staticFieldsSize != 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if(dvmHasCARMemberAnnotation(&(newClass->sfields[i]).field)){
				//LOGD("field has CARMember %s", (newClass->ifields[i]).field.name);
				dvmSetStaticCARMemberHook(&newClass->sfields[i]);
			}
        }
    }
    ...
    if (pHeader->instanceFieldsSize != 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (dvmHasCARMemberAnnotation(&(newClass->ifields[i]).field)) {
				//LOGD("field has CARMember %s", (newClass->ifields[i]).field.name);
				if (!dvmSetCARMemberHook(&newClass->ifields[i]))
					LOGE("dvmSetCARMemberHook Failed");
			} else if ((newClass->ifields[i]).field.signature[0] == 'L') {

                DexClassDef  *pClassDef;
                u4            u4tmp;

                pClassDef = (DexClassDef*)dexFindClass(pDexFile, (newClass->ifields[i]).field.signature);
                if (pClassDef == NULL)
                    continue;

                u4tmp = newClass->pDvmDex->pDexFile->classIdxDelegates;
                if (pClassDef->classIdx == u4tmp || pClassDef->superclassIdx  == u4tmp ) {
    			    newClass->ifields[i].interfaceIndexOfSetter = 0xFFFF;
    				if (!dvmSetCARMemberHook(&newClass->ifields[i])) {
    					LOGD("dvmSetCARMemberHook Failed");
    					newClass->ifields[i].interfaceIndexOfSetter = 0;
    				}
			    } else {
                    u4tmp = newClass->pDvmDex->pDexFile->classIdxCallbacks;
			        if (pClassDef->classIdx == u4tmp || pClassDef->superclassIdx  == u4tmp ) {
        			    newClass->ifields[i].interfaceIndexOfSetter = 0xFFFE;
        				if (!dvmSetCARMemberHook(&newClass->ifields[i])) {
        					LOGD("dvmSetCARMemberHook Failed");
        					newClass->ifields[i].interfaceIndexOfSetter = 0;
        				}
    ...
    if (IS_CLASS_FLAG_SET(newClass, CLASS_CAR_CLASS) && gDvm.shouldCARWorking) {
        dvmSetCARmethodInfoForJavaMethod(newClass);
    }
...
#define NULL_AND_LINEAR_FREE(p) \
    ...
    /**
      * for java&CAR support
      * dvmReleaseTrackedAlloc() will use obj->clazz and obj->clazz->super
      * we have to set obj.clazz a temp valid value
      */
...
static void freeMethodInnards(Method* meth)
{
	...
	if (IS_CLASS_FLAG_SET(clazz, CLASS_CAR_CLASS|CLASS_CAR_INTERFACE)) {
		...
		if (pCARMethodInfo != NULL) {
			meth->pCARMethodInfo = NULL;

			dvmReleaseCARMethodInfo(meth);

			if (pCARMethodInfo->regTypes != NULL) {
				void* regTypes = pCARMethodInfo->regTypes;
				pCARMethodInfo->regTypes = NULL;
				dvmLinearFree(clazz->classLoader, regTypes);
			}
			if (pCARMethodInfo->regExtraData != NULL) {
				void* regExtraData = pCARMethodInfo->regExtraData;
				pCARMethodInfo->regExtraData = NULL;
    ...
    if (pDexCode != NULL) {
        ...
        if (dvmIsNativeMethod(meth)) {
			...
			if (!IS_CLASS_FLAG_SET(clazz, CLASS_CAR_CLASS|CLASS_CAR_INTERFACE)) {
		        ...
		        meth->jniArgInfo = computeCARJniArgInfo(&meth->prototype);
			}
        }
    }
}
 ...
 *   H - target-specific hints
 *
 * This info is used at invocation time by dvmPlatformCARInvoke.  In most
 * cases, the target-specific hints allow dvmPlatformCARInvoke to avoid
 * having to fully parse the signature.
 *
 * The return-type bits are always set, even if target-specific hint bits
 * are unavailable.
...
static int computeCARJniArgInfo(const DexProto* proto)
{
    ...
    hints = dvmPlatformCARInvokeHints(proto);
    ...
    for (i = 0; i < clazz->ifieldCount; i++) {
        ...
        */
        if (pField->interfaceIndexOfSetter == 0xFFFF || pField->interfaceIndexOfSetter == 0xFFFE) {
                ...
                //reserve a u4 space for older data
                fieldOffset += sizeof(u4);

                SET_CLASS_FLAG(clazz, CLASS_CAR_NEEDCLEAN);
        }
