...
static const char* kDescrCARClass   = "Ldalvik/annotation/ElastosClass;";
static const char* kDescrCARInterface   = "Ldalvik/annotation/ElastosInterface;";
static const char* kDescrCARMember   = "Ldalvik/annotation/Property;";
static const char* kDescrCARMethod   = "Ldalvik/annotation/Method;";
...
/*
 *Runturn true if the specified class has ElastosClass annotation
 */
bool dvmHasCARClassAnnotation(const ClassObject* clazz)
{
	const DexAnnotationSetItem* pAnnoSet = NULL;
	...
	if(pAnnoSet != NULL){
		const DexAnnotationItem* pAnnoItem;
		pAnnoItem = searchAnnotationSet(clazz, pAnnoSet, kDescrCARClass, kDexVisibilityRuntime);
		if(pAnnoItem != NULL)
			return true;
	}
	return false;
...
/*
 *Runturn true if the specified class has CARInterface annotation
 */
bool dvmHasCARInterfaceAnnotation(const ClassObject* clazz)
{
	const DexAnnotationSetItem* pAnnoSet = NULL;
	...
	if(pAnnoSet != NULL){
		const DexAnnotationItem* pAnnoItem;
		pAnnoItem = searchAnnotationSet(clazz, pAnnoSet, kDescrCARInterface, kDexVisibilityRuntime);
		if(pAnnoItem != NULL)
			return true;
	}
	return false;
...
char* dvmGetCARClassAnnotationValue(const ClassObject* clazz, char* key)
{
	const DexAnnotationSetItem* pAnnoSet = NULL;
	...
	if(pAnnoSet != NULL){
		...
		if(pAnnoItem != NULL){
			const u1* ptr;
			ptr = searchEncodedAnnotation(clazz, pAnnoItem->annotation, key);
			if(ptr == NULL){
				LOGW("annotation lacks %s member\n", key);
				return NULL;
			}
			AnnotationValue avalue;
			if(!processAnnotationValue(clazz, &ptr, &avalue, kAllRaw))
				return NULL;
			return (char*)dexStringById(clazz->pDvmDex->pDexFile, avalue.value.i);
		}
	}
	return NULL;
}

char* dvmGetCARInterfaceAnnotationValue(const ClassObject* clazz, char* key)
{
	const DexAnnotationSetItem* pAnnoSet = NULL;
...
/*
 *Runturn true if the specified Field has CARMember annotation
 */
bool dvmHasCARMemberAnnotation(const Field* field)
{
	const DexAnnotationSetItem* pAnnoSet;
	...
	if (pAnnoSet != NULL) {
		const DexAnnotationItem* pAnnoItem;
		pAnnoItem = searchAnnotationSet(clazz, pAnnoSet, kDescrCARMember, kDexVisibilityRuntime);
		if (pAnnoItem != NULL)
			return true;
	}
	return false;
...
char* dvmGetCARMemberAnnotationValue(const Field* field, char* key)
{
	const DexAnnotationSetItem* pAnnoSet;
...
char* dvmGetCARMethodAnnotationValue(const Method* method, char* key)
{
	const DexAnnotationSetItem* pAnnoSet;
	...
	if (pAnnoSet != NULL) {
		const DexAnnotationItem* pAnnoItem;
		pAnnoItem = searchAnnotationSet(clazz, pAnnoSet, kDescrCARMethod, kDexVisibilityRuntime);
		if (pAnnoItem != NULL) {
			const u1* ptr;
			ptr = searchEncodedAnnotation(clazz, pAnnoItem->annotation, key);
			if (ptr == NULL) {
