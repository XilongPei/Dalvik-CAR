    ...
    const u1* buf, int dataLen, ExpandBuf* pReply)
{
    ...
    objectId = dvmReadObjectId(&buf);
	Object* Java_Obj = (Object*)(u4)objectId;
...
#define MAX_PSEUDO_FIELD_ID    0x800
    	if (fieldId <= MAX_PSEUDO_FIELD_ID) {
    	    int          j;
            ClassObject *clz;
            u4          *pCARObject;
            Object      *CARObjDesc_Obj;

	    for (j = 0, clz = Java_Obj->clazz;  clz != NULL;  clz = clz->super) {
                if (IS_CLASS_FLAG_SET(clz, CLASS_CAR_CLASS)) {
                    if ((MAX_PSEUDO_FIELD_ID - j) == (int)fieldId) {
    	                pCARObject = (u4*)(*(u4*)(((char*)Java_Obj) + Java_Obj->clazz->objectSize + sizeof(u4)*j));
                        break;
                    }
                    j++;
                }
			...
			jclass clzDesc = dvmFindClass("Lelastos/CarDebugger/CARObjDesc;",NULL);
			Method* meth = dvmFindDirectMethodByDescriptor(clzDesc, "<init>", "()V");
			CARObjDesc_Obj = (Object*) dvmAllocObject(clzDesc, ALLOC_DONT_TRACK);
			JValue unused;
			dvmCallMethod(self, meth, CARObjDesc_Obj, &unused);
            dvmSetCARInspectObjMoreInfo(Java_Obj,CARObjDesc_Obj, pCARObject, j);
            dvmSetObjectId(ptr, (ObjectId)(u4)CARObjDesc_Obj);
...
//-----------------------------------------------------------------------------
		...
		{
			ClassObject* fieldClazz = dvmFindClass(field->signature,field->clazz->classLoader);
			if(strcmp(fieldClazz->descriptor,"Ljava/lang/Callbacks;")==0 || \
			  (fieldClazz->super!=NULL && strcmp(fieldClazz->super->descriptor,"Ljava/lang/Callbacks;")==0)) {
				ArrayObject* CallbackObjDesc_Obj;
				Object* Callback_Obj = NULL;

				Callback_Obj = dvmGetFieldObject(Java_Obj,((InstField*)field)->byteOffset);
        		...
        		u4      *thisObjs;
        		u4       icount;
				CallbackMethods = (u4*)(((ArrayObject*)dvmGetFieldObject((const Object*)Callback_Obj,
                                            gDvm.offJavaLangDelegate_value))->contents);
				thisObjs = (u4*)(((ArrayObject*)dvmGetFieldObject((const Object*)Callback_Obj,
                                            gDvm.offJavaLangDelegate_thisObj))->contents);
				icount = dvmGetFieldInt((const Object*)Callback_Obj, gDvm.offJavaLangDelegate_count);

				ClassObject* classArrayClass = dvmFindArrayClass("[Lelastos/CarDebugger/CallbackMethod;", NULL);
				CallbackObjDesc_Obj = dvmAllocArrayByClass(classArrayClass,icount,ALLOC_DEFAULT);

				fieldTag=JT_ARRAY;
            	expandBufAdd1(pReply, fieldTag);
            	width = dvmDbgGetTagWidth(fieldTag);
            	ptr=expandBufAddSpace(pReply,width);
				//jclass clzCallbackMethod=(*pJNIEnv)->FindClass(pJNIEnv,"elastos/CarDebugger/CallbackMethod");
				//jmethodID methodID_init = (*pJNIEnv)->GetMethodID(pJNIEnv, clzCallbackMethod, "<init>", "()V");
				//jmethodID methodID_setClazz = (*pJNIEnv)->GetMethodID(pJNIEnv, clzCallbackMethod, "setClazz", "(Ljava/lang/String;)V");
				//jmethodID methodID_setName = (*pJNIEnv)->GetMethodID(pJNIEnv, clzCallbackMethod, "setName", "(Ljava/lang/String;)V");

				jclass clzCallbackMethod = dvmFindClass("Lelastos/CarDebugger/CallbackMethod;",NULL);
				Method* meth_init = dvmFindDirectMethodByDescriptor(clzCallbackMethod, "<init>", "()V");
				Method* meth_setClazz = dvmFindVirtualMethodByDescriptor(clzCallbackMethod, "setClazz", "(Ljava/lang/String;)V");
				Method* meth_setName = dvmFindVirtualMethodByDescriptor(clzCallbackMethod, "setName", "(Ljava/lang/String;)V");
				...
				for (i = 0; i < (int)icount; i++) {
					...
					if (CallbackMethods[i] == 0) {
						if (obj != 0) {
							CallbackMethod_Obj = (Object*) dvmAllocObject(clzCallbackMethod, ALLOC_DONT_TRACK);
							dvmCallMethod(self, meth_init, CallbackMethod_Obj, &unused);

							StringObject *str = dvmCreateStringFromCstr(obj->clazz->descriptor);
    						dvmCallMethod(self, meth_setClazz, CallbackMethod_Obj, &unused, str);
    						dvmReleaseTrackedAlloc((Object*)str, self);

							char buf[256];
							str = dvmCreateStringFromCstr(strcat(strcpy(buf, "On"), field->name));
    						dvmCallMethod(self, meth_setName, CallbackMethod_Obj, &unused, str);
    						dvmReleaseTrackedAlloc((Object*)str, self);
							...
							StringObject *str = dvmCreateStringFromCstr(Callback_Obj->clazz->descriptor);
    						dvmCallMethod(self, meth_setClazz, CallbackMethod_Obj, &unused, str);
    						dvmReleaseTrackedAlloc((Object*)str, self);
						...
						if (obj != 0) {
							...
							str = dvmCreateStringFromCstr(((Method*)CallbackMethods[i])->name);
							dvmCallMethod(self, meth_setName, CallbackMethod_Obj, &unused, str);
    						dvmReleaseTrackedAlloc((Object*)str, self);
							...
							StringObject *str = dvmCreateStringFromCstr(((Method*)CallbackMethods[i])->clazz->descriptor);
    						dvmCallMethod(self, meth_setClazz, CallbackMethod_Obj, &unused, str);
    						dvmReleaseTrackedAlloc((Object*)str, self);
					...
					CallbackObjDesc_Obj->contents[i] = (u4)CallbackMethod_Obj;
				}
				dvmSetObjectId(ptr, (ObjectId)(u4)CallbackObjDesc_Obj);
            	return ERR_NONE;

			} else if (strcmp(fieldClazz->descriptor,"Ljava/lang/Delegates;")==0 || \
			  (fieldClazz->super!=NULL && strcmp(fieldClazz->super->descriptor,"Ljava/lang/Delegates;")==0)) {
				;
			}
		}
