...
const char *prefixCARInterfaceForWrapper = "Wrapper";

static void *CARProbe(JNIEnv* env, jobject obj, const char *clzName, char *ifxName);
...
/*
 * data type complex defination in Dalvik&CAR
 *                     1                   2                   3
 ...
 *                                                           isParamPrimitive
 */
#define paramInOutAttribute(p) ((p>>30)&0x0003)
#define isParamPrimitive(p) ((p>>29)&0x0001)
#define getParamType(p) ((p>>16)&0x000f)
#define getParamTypeExtra(p) ((p>>8)&0x000f)

/* for CAR support
 * function prototype is dvmPlatformInvoke()
 * it is implemented in Call.c Call386ABI.S
 *
 * Bridge to calling a CAR JNI function.  This ideally gets some help from
 ...
 */
void dvmCallCARJNIMethod(const u4* args, JValue* pResult, const Method* method,
    Thread* self)
{
    int          oldStatus;
    int          i;
	...
	if (dvmIsStaticMethod(method)) {
        ...
        clz = method->clazz;
        pCARObject = (u4*)clz->pCARSingletonObject;
    	...
    	pCARObject = (u4*)*(u4*)(((char*)object) + method->clazz->objectSize + sizeof(u4)*i);
        LOGD("pCARObject %p, method->jniArgInfo %x, method->insSize %d, args %p, method->shorty %s, (void*)method->insns %p, &out %p",
    		pCARObject, method->jniArgInfo, method->insSize, args, method->shorty, (void*)method->insns, pResult);
    	LOGD("self %p clazz %p", self->jniEnv, method->clazz);
	...
	if (!method->pCARMethodInfo->needCopy){
		ec = dvmPlatformCARInvoke(pCARObject+method->pCARMethodInfo->interfaceIndex, method->jniArgInfo, insSize, args, method->shorty,
		                                    (void*)method->insns, pResult);
        if (ec) {
            char  buf[256];

            snprintf(buf, sizeof(buf), "call CAR method \"%s\" error, ECode=%0x", method->name, ec);
            dvmThrowException("Ljava/lang/IllegalAccessError;", buf);
        }
	} else {
		u4* regTypes = method->pCARMethodInfo->regTypes;
		u4* regExtraData = method->pCARMethodInfo->regExtraData;
		u4* argsCopy = alloca(method->insSize*sizeof(u4));
		int i;
		JValue* pResultCopy = NULL;

        args--;
		for (i = 1; i < method->insSize+1; i++)
		{
			u4 paramType = regTypes[i];
			if (isParamPrimitive(paramType)) {
				//if the type is primitive type, we just copy the type to the call stack.
				argsCopy[i] = args[i];
				continue;
			}

			if (paramInOutAttribute(paramType) == carParamIOAttribute_In) {
				switch (getParamType(paramType)) {

				    //CAR don't allow StringBuf work as [in] parameter
					case kTypeString: {
						char *s = dvmCreateCstrFromString((StringObject*)args[i]);
						if (s == NULL) {
                            char  buf[256];

                            snprintf(buf, sizeof(buf), "call CAR method \"%s\"", method->name);
                            dvmThrowException("Ljava/lang/NullPointerException;", buf);
							LOGE("Ljava/lang/NullPointerException;");
						}
						argsCopy[i] = (u4)dvmElapiNewString(s);
						break;
					}

					case kTypeArrayOf: {

					    jsize size = ((ArrayObject*)args[i])->length;

						switch (getParamTypeExtra(paramType)) {
							case kTypeBoolean:
								argsCopy[i] = (u4)dvmElapiArrayNewBoolean((jboolean*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeByte:
								argsCopy[i] = (u4)dvmElapiArrayNewByte((unsigned char *)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeChar:{
								jchar *pChar = (jchar*)((ArrayObject*)args[i])->contents;
								char  *s = alloca(size*sizeof(char));
								int j;
								for(j=0; j<size; j++)
								{
									s[j] = pChar[j];
								}
								argsCopy[i] = (u4)dvmElapiArrayNewChar(s,size);
								break;
							}
							case kTypeShort:
								argsCopy[i] = (u4)dvmElapiArrayNewShort((short*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeInt:
								argsCopy[i] = (u4)dvmElapiArrayNewInt((jint*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeLong:
								argsCopy[i] = (u4)dvmElapiArrayNewLong((jlong*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeFloat:
								argsCopy[i] = (u4)dvmElapiArrayNewFloat((jfloat*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeDouble:
								argsCopy[i] = (u4)dvmElapiArrayNewDouble((jdouble*)((ArrayObject*)args[i])->contents, size);
								break;
					...
					case kTypeBufferOf:{
						...
						switch (getParamTypeExtra(paramType)) {
							case kTypeBoolean:
								argsCopy[i] = (u4)dvmElapiBufferNewBoolean((jboolean*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeByte:
								argsCopy[i] = (u4)dvmElapiBufferNewByte((unsigned char *)((ArrayObject*)args[i])->contents, size);
								break;
							...
							case kTypeChar: {
								...
								argsCopy[i] = (u4)dvmElapiBufferNewChar(s,size);
								break;
							}
							case kTypeShort:
								argsCopy[i] = (u4)dvmElapiBufferNewShort((jshort*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeInt:
								argsCopy[i] = (u4)dvmElapiBufferNewInt((jint*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeLong:
								argsCopy[i] = (u4)dvmElapiBufferNewLong((jlong*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeFloat:
								argsCopy[i] = (u4)dvmElapiBufferNewFloat((jfloat*)((ArrayObject*)args[i])->contents, size);
								break;

							case kTypeDouble:
								argsCopy[i] = (u4)dvmElapiBufferNewDouble((jdouble*)((ArrayObject*)args[i])->contents, size);
								break;
    				...
    				case kTypeEnum:
					{
						...
						char *s = dvmCreateCstrFromString((StringObject*)rt.l);
						if (s == NULL) {
                            char  buf[256];
						...
						int j = dvmGetCAREnumInt((void*)regExtraData[i], s);
						free(s);
						argsCopy[i] = j;
						break;
					}
				    case kTypeInterface:
				    {
                        ...
                        for (i = 0, clz = obj->clazz;  clz != NULL;  clz = clz->super) {
                            if (IS_CLASS_FLAG_SET(clz, CLASS_CAR_CLASS)) {
			                    ifc = (u4)ProbeInterface(*(u4*)(((char*)obj) + obj->clazz->objectSize + sizeof(u4)*i), regExtraData[i]);
			                    if (ifc != 0)
			                        break;
			                    i++;
			                } else if (IS_CLASS_FLAG_SET(clz, CLASS_CAR_INTERFACE)) {
                                ifc = *(u4*)(((char*)obj) + obj->clazz->objectSize);

                                //check the two interfaces, wether they are the same
                                if (dvmChkInterfaceWithInterfaceID(ifc, regExtraData[i])) {
			...
			} else if (paramInOutAttribute(paramType) == carParamIOAttribute_CallerAllocOut) {

				int outsSize = method->outsSize;
				...
				switch (getParamType(paramType)) {
				    ...
				    case kTypeInterface:
						break;
					case kTypeStringBuffer:{
						pResultCopy = pResult;
						pResult = dvmElapiNewStringBuf(outsSize);
						break;
					}
					case kTypeArrayOf: {
					    pResultCopy = pResult;
						switch (getParamTypeExtra(paramType)) {
							case kTypeBoolean:
								pResult = dvmElapiArrayNewBoolean(NULL,outsSize);
								break;
							case kTypeByte:
								pResult = dvmElapiArrayNewByte(NULL,outsSize);
								break;
							case kTypeChar:
								pResult = dvmElapiArrayNewChar(NULL,outsSize);
								break;
							case kTypeShort:
								pResult = dvmElapiArrayNewShort(NULL,outsSize);
								break;
							case kTypeInt:
								pResult = dvmElapiArrayNewInt(NULL,outsSize);
								break;
							case kTypeLong:
								pResult = dvmElapiArrayNewLong(NULL,outsSize);
								break;
							case kTypeFloat:
								pResult = dvmElapiArrayNewFloat(NULL,outsSize);
								break;
							case kTypeDouble:
								pResult = dvmElapiArrayNewDouble(NULL,outsSize);
								break;
							default:
								LOGE("This type of ArrayOf not implement");
							}
					...
					case kTypeBufferOf: {
						...
						switch (getParamTypeExtra(paramType)) {
							case kTypeBoolean:
								pResult = dvmElapiBufferNewBoolean(NULL,outsSize);
								break;
							case kTypeByte:
								pResult = dvmElapiBufferNewByte(NULL,outsSize);
								break;
							case kTypeChar:
								pResult = dvmElapiBufferNewChar(NULL,outsSize);
								break;
							case kTypeShort:
								pResult = dvmElapiBufferNewShort(NULL,outsSize);
								break;
							case kTypeInt:
								pResult = dvmElapiBufferNewInt(NULL,outsSize);
								break;
							case kTypeLong:
								pResult = dvmElapiBufferNewLong(NULL,outsSize);
								break;
							case kTypeFloat:
								pResult = dvmElapiBufferNewFloat(NULL,outsSize);
								break;
							case kTypeDouble:
								pResult = dvmElapiBufferNewDouble(NULL,outsSize);
								break;
							default:
								LOGE("This type of BufferOf not implement");
						}
					...
					default:
						LOGE("This out type not implement");
				}
			}
		}

		ec = dvmPlatformCARInvoke(pCARObject+method->pCARMethodInfo->interfaceIndex, method->jniArgInfo, insSize, argsCopy+1, method->shorty,
		                                    (void*)method->insns, pResult);
        if (ec) {
            char  buf[256];
        ...
        //
        //continue, even failed in CAR, for some parameter need to be freed
        //
        for (i = 0;  i < method->insSize+1;  i++)
		{
			u4 paramType = regTypes[i];
			...
			if (paramInOutAttribute(paramType) == carParamIOAttribute_In) {
			    switch (getParamType(paramType)) {
				    case kTypeString:
						free((void*)argsCopy[i]);
						break;
					case kTypeArrayOf:
						switch (getParamTypeExtra(paramType)) {
							case kTypeBoolean:
								dvmElapiArrayFreeBoolean((void*)argsCopy[i]);
								break;
							case kTypeByte:
								dvmElapiArrayFreeByte((void*)argsCopy[i]);
								break;
							case kTypeChar:
								dvmElapiArrayFreeChar((void*)argsCopy[i]);
								break;
							case kTypeShort:
								dvmElapiArrayFreeShort((void*)argsCopy[i]);
								break;
							case kTypeInt:
								dvmElapiArrayFreeInt((void*)argsCopy[i]);
								break;
							case kTypeLong:
								dvmElapiArrayFreeLong((void*)argsCopy[i]);
								break;
							case kTypeFloat:
								dvmElapiArrayFreeFloat((void*)argsCopy[i]);
								break;
							case kTypeDouble:
								dvmElapiArrayFreeDouble((void*)argsCopy[i]);
								break;
							default:
								LOGE("This type of ArrayOf not implement");
						}
					...
					case kTypeBufferOf:
						switch (getParamTypeExtra(paramType)) {
							case kTypeBoolean:
								dvmElapiBufferFreeBoolean((void*)argsCopy[i]);
								break;
							case kTypeByte:
								dvmElapiBufferFreeByte((void*)argsCopy[i]);
								break;
							case kTypeChar:
								dvmElapiBufferFreeChar((void*)argsCopy[i]);
								break;
							case kTypeShort:
								dvmElapiBufferFreeShort((void*)argsCopy[i]);
								break;
							case kTypeInt:
								dvmElapiBufferFreeInt((void*)argsCopy[i]);
								break;
							case kTypeLong:
								dvmElapiBufferFreeLong((void*)argsCopy[i]);
								break;
							case kTypeFloat:
								dvmElapiBufferFreeFloat((void*)argsCopy[i]);
								break;
							case kTypeDouble:
								dvmElapiBufferFreeDouble((void*)argsCopy[i]);
								break;
							default:
								LOGE("This type of BufferOf not implement");
						}
			    ...
			    if (ec) {
    				switch(getParamType(paramType)){
    					case kTypeString:
    						pResult->l = NewStringUTF(self->jniEnv, "");
    						break;
    					case kTypeStringBuffer:{
    						//jclass cls = FindClass(self->jniEnv, "java/lang/StringBuffer");
    						//jmethodID methodID = GetMethodID(self->jniEnv, cls, "<init>", "(Ljava/lang/String;)V");
    						jstring js = NewStringUTF(self->jniEnv, "");
    						//jobject obj = NewObject(self->jniEnv, cls, methodID, js);
    						//jobject obj = NewObject(self->jniEnv, (jclass)gDvm.classJavaLangStringBuffer,
    						//                        (jmethodID)gDvm.methJavaLangStringBuffer, js);
                            Object *obj = dvmClassNewInstance(gDvm.classJavaLangStringBuffer,gDvm.methJavaLangStringBuffer, js);
    						pResult = pResultCopy;
    						pResult->l = obj;
    						break;
    					}
    					...
    					case kTypeArrayOf:
    					{
    						switch (getParamTypeExtra(paramType)) {
    							case kTypeBoolean:{
    								jbooleanArray arrayZ = NewBooleanArray(self->jniEnv, 0);
    								pResultCopy->l = arrayZ;
    								break;
    							}
    							case kTypeByte:{
    								jbyteArray arrayB = NewByteArray(self->jniEnv, 0);
    								pResultCopy->l = arrayB;
    								break;
    							}
    							case kTypeChar:{
    								jcharArray arrayC = NewCharArray(self->jniEnv, 0);
    								pResultCopy->l = arrayC;
    								break;
    							}
    							case kTypeShort:{
    								jshortArray arrayS = NewShortArray(self->jniEnv, 0);
    								pResultCopy->l = arrayS;
    								break;
    							}
    							case kTypeInt:{
    								jintArray arrayI = NewIntArray(self->jniEnv, 0);
    								pResultCopy->l = arrayI;
    								break;
    							}
    							case kTypeLong:{
    								jlongArray arrayJ = NewLongArray(self->jniEnv, 0);
    								pResultCopy->l = arrayJ;
    								break;
    							}
    							case kTypeFloat:{
    								jfloatArray arrayF = NewFloatArray(self->jniEnv, 0);
    								pResultCopy->l = arrayF;
    								break;
    							}
    							case kTypeDouble:{
    								jdoubleArray arrayD = NewDoubleArray(self->jniEnv, 0);
    								pResultCopy->l = arrayD;
    								break;
    							}
    							default:
    								LOGE("This type of ArrayOf not implement");
    					...
    					default:
    						LOGE("out params not implement");
    				}
			    } else {
    				switch (getParamType(paramType)) {
    					case kTypeString: {
    					    char  *s = (char*)(pResult->l);
    						pResult->l = NewStringUTF(self->jniEnv, s);
    						free(s);
    						break;
    					}
    					case kTypeStringBuffer:{
    						//jclass cls = FindClass(self->jniEnv, "java/lang/StringBuffer");
    						//jmethodID methodID = GetMethodID(self->jniEnv, cls, "<init>", "(Ljava/lang/String;)V");
    						jstring js = NewStringUTF(self->jniEnv, dvmElapiStringBufGetPayload(pResult));
    						//jobject obj = NewObject(self->jniEnv, cls, methodID, js);
    						//jobject obj = NewObject(self->jniEnv, (jclass)gDvm.classJavaLangStringBuffer,
    						//                        (jmethodID)gDvm.methJavaLangStringBuffer, js);
                            Object *obj = dvmClassNewInstance(gDvm.classJavaLangStringBuffer,gDvm.methJavaLangStringBuffer, js);
    						dvmElapiFreeStringBuf(pResult);
    						pResult = pResultCopy;
    						pResult->l = obj;
    						break;
    					}
    					case kTypeArrayOf:{
    						switch (getParamTypeExtra(paramType)) {
    							case kTypeBoolean:{
    								jboolean* pBoolean = dvmElapiArrayGetPayloadBoolean(pResult);
    								jsize size = dvmElapiArrayGetLengthBoolean(pResult);
    								jbooleanArray arrayZ = NewBooleanArray(self->jniEnv, size);
    								SetBooleanArrayRegion(self->jniEnv, arrayZ, 0, size, pBoolean);
    								dvmElapiArrayFreeBoolean(pResult);
    								pResultCopy->l = arrayZ;
    								break;
    							}
    							case kTypeByte:{
    								jbyte* pByte = (jbyte*)dvmElapiArrayGetPayloadByte(pResult->l);
    								jsize size = dvmElapiArrayGetLengthByte(pResult);
    								jbyteArray arrayB = NewByteArray(self->jniEnv, size);
    								SetByteArrayRegion(self->jniEnv, arrayB, 0, size, pByte);
    								dvmElapiArrayFreeByte(pResult);
    								pResultCopy->l = arrayB;
    								break;
    							}
    							case kTypeChar:{
    								jsize size = dvmElapiArrayGetLengthChar(pResult);
    								char* s = dvmElapiArrayGetPayloadChar(pResult);
    								int j;
    								jchar* pChar = alloca(size*sizeof(char));
    								for(j=0; j<size; j++){
    									pChar[j] = s[j];
    								}
    								jcharArray arrayC = NewCharArray(self->jniEnv, size);
    								SetCharArrayRegion(self->jniEnv, arrayC, 0, size, pChar);
    								dvmElapiArrayFreeChar(pResult);
    								pResultCopy->l = arrayC;
    								break;
    							}
    							case kTypeShort:{
    								jshort* pShort = dvmElapiArrayGetPayloadShort(pResult);
    								jsize size = dvmElapiArrayGetLengthShort(pResult);
    								jshortArray arrayS = NewShortArray(self->jniEnv, size);
    								SetShortArrayRegion(self->jniEnv, arrayS, 0, size, pShort);
    								dvmElapiArrayFreeShort(pResult);
    								pResultCopy->l = arrayS;
    								break;
    							}
    							case kTypeInt:{
    								jint* pInt = dvmElapiArrayGetPayloadInt(pResult);
    								jsize size = dvmElapiArrayGetLengthInt(pResult);
    								jintArray arrayI = NewIntArray(self->jniEnv, size);
    								SetIntArrayRegion(self->jniEnv, arrayI, 0, size, pInt);
    								dvmElapiArrayFreeInt(pResult);
    								pResultCopy->l = arrayI;
    								break;
    							}
    							case kTypeLong:{
    								jlong* pLong = dvmElapiArrayGetPayloadLong(pResult);
    								jsize size = dvmElapiArrayGetLengthLong(pResult);
    								jlongArray arrayJ = NewLongArray(self->jniEnv, size);
    								SetLongArrayRegion(self->jniEnv, arrayJ, 0, size, pLong);
    								dvmElapiArrayFreeLong(pResult);
    								pResultCopy->l = arrayJ;
    								break;
    							}
    							case kTypeFloat:{
    								jfloat* pFloat = dvmElapiArrayGetPayloadFloat(pResult);
    								jsize size = dvmElapiArrayGetLengthFloat(pResult);
    								jfloatArray arrayF = NewFloatArray(self->jniEnv, size);
    								SetFloatArrayRegion(self->jniEnv, arrayF, 0, size, pFloat);
    								dvmElapiArrayFreeFloat(pResult);
    								pResultCopy->l = arrayF;
    								break;
    							}
    							case kTypeDouble:{
    								jdouble* pDouble = dvmElapiArrayGetPayloadDouble(pResult);
    								jsize size = dvmElapiArrayGetLengthDouble(pResult);
    								jdoubleArray arrayD = NewDoubleArray(self->jniEnv, size);
    								SetDoubleArrayRegion(self->jniEnv, arrayD, 0, size, pDouble);
    								dvmElapiArrayFreeDouble(pResult);
    								pResultCopy->l = arrayD;
    								break;
    							}
    							default:
    					...
    					case kTypeBufferOf:{
    						switch(getParamTypeExtra(paramType)){
    							case kTypeBoolean:{
    								jboolean* pBoolean = dvmElapiBufferGetPayloadBoolean(pResult);
    								jsize size = dvmElapiBufferGetLengthBoolean(pResult);
    								jbooleanArray BufferZ = NewBooleanArray(self->jniEnv, size);
    								SetBooleanArrayRegion(self->jniEnv, BufferZ, 0, size, pBoolean);
    								dvmElapiBufferFreeBoolean(pResult);
    								pResultCopy->l = BufferZ;
    								break;
    							}
    							case kTypeByte:{
    								jbyte* pByte = (jbyte*)dvmElapiBufferGetPayloadByte(pResult);
    								jsize size = dvmElapiBufferGetLengthByte(pResult);
    								jbyteArray BufferB = NewByteArray(self->jniEnv, size);
    								SetByteArrayRegion(self->jniEnv, BufferB, 0, size, pByte);
    								dvmElapiBufferFreeByte(pResult);
    								pResultCopy->l = BufferB;
    								break;
    							}
    							case kTypeChar:{
    								jsize size = dvmElapiBufferGetLengthChar(pResult);
    								char* s = dvmElapiBufferGetPayloadChar(pResult);
    								int j;
    								jchar* pChar = alloca(size*sizeof(char));
    								for(j=0; j<size; j++){
    									pChar[j] = s[j];
    								}
    								jcharArray BufferC = NewCharArray(self->jniEnv, size);
    								SetCharArrayRegion(self->jniEnv, BufferC, 0, size, pChar);
    								dvmElapiBufferFreeChar(pResult);
    								pResultCopy->l = BufferC;
    								break;
    							}
    							case kTypeShort:{
    								jshort* pShort = dvmElapiBufferGetPayloadShort(pResult);
    								jsize size = dvmElapiBufferGetLengthShort(pResult);
    								jshortArray BufferS = NewShortArray(self->jniEnv, size);
    								SetShortArrayRegion(self->jniEnv, BufferS, 0, size, pShort);
    								dvmElapiBufferFreeShort(pResult);
    								pResultCopy->l = BufferS;
    								break;
    							}
    							case kTypeInt:{
    								jint* pInt = dvmElapiBufferGetPayloadInt(pResult);
    								jsize size = dvmElapiBufferGetLengthInt(pResult);
    								jintArray BufferI = NewIntArray(self->jniEnv, size);
    								SetIntArrayRegion(self->jniEnv, BufferI, 0, size, pInt);
    								dvmElapiBufferFreeInt(pResult);
    								pResultCopy->l = BufferI;
    								break;
    							}
    							case kTypeLong:{
    								jlong* pLong = dvmElapiBufferGetPayloadLong(pResult);
    								jsize size = dvmElapiBufferGetLengthLong(pResult);
    								jlongArray BufferJ = NewLongArray(self->jniEnv, size);
    								SetLongArrayRegion(self->jniEnv, BufferJ, 0, size, pLong);
    								dvmElapiBufferFreeLong(pResult);
    								pResultCopy->l = BufferJ;
    								break;
    							}
    							case kTypeFloat:{
    								jfloat* pFloat = dvmElapiBufferGetPayloadFloat(pResult);
    								jsize size = dvmElapiBufferGetLengthFloat(pResult);
    								jfloatArray BufferF = NewFloatArray(self->jniEnv, size);
    								SetFloatArrayRegion(self->jniEnv, BufferF, 0, size, pFloat);
    								dvmElapiBufferFreeFloat(pResult);
    								pResultCopy->l = BufferF;
    								break;
    							}
    							case kTypeDouble:{
    								jdouble* pDouble = dvmElapiBufferGetPayloadDouble(pResult);
    								jsize size = dvmElapiBufferGetLengthDouble(pResult);
    								jdoubleArray BufferD = NewDoubleArray(self->jniEnv, size);
    								SetDoubleArrayRegion(self->jniEnv, BufferD, 0, size, pDouble);
    								dvmElapiBufferFreeDouble(pResult);
    								pResultCopy->l = BufferD;
    								break;
    							}
    							default:
    								LOGE("This type of BufferOf not implement");
        				...
        				case kTypeEnum:
    					{
    						...
    						char *s = dvmGetCAREnumString((void*)regExtraData[i], pResult->i);
    						//dvmReleaseCARClassObject(regExtraData[i]);

    						if (s != NULL) {
        						clazz = dvmGetBoxedReturnType(method);
        						Method* methodEnum = dvmFindDirectMethodHierByDescriptor(clazz, "valueOf", "(Ljava/lang/Class;Ljava/lang/String;)Ljava/lang/Enum;");

        						if (methodEnum == NULL) {
        						    LOGE("This type of enum not implement");
        						    pResult->i = -1;
        						} else {
        						    dvmCallMethod(self, methodEnum, NULL, &rt, clazz, NewStringUTF(self->jniEnv, s));
        						    pResult->l = rt.l;
        						}
    						} else {
    						    pResult->i = -1;
    				    ...
    				    case kTypeInterface:
    				    {
    				    	//LOGD("kTypeInterface%p %p",pResult, pResult->l);
    				    	//char*s = dvmReflectClassName(pResult->l);
    				    	//LOGD("s %s", s);
    						char        *desc = (char*)dexProtoGetReturnType(&method->prototype);
    						char        *cp;
    						ClassObject *clazz;
    						int          len;
    						char        *wrapperFunc;
    						...
    						if (strcmp(desc, "Ljava/lang/Object;") == 0) {
        						...
        						if (cp != desc) {
        						    ...
        						    strcat(wrapperFunc, (char *)regExtraData[i]);
        						} else {
    						...
    						/*
    						 * we have already check it in dvmSetCARMethodInfo()
    						 *
    						if (clazz == NULL) {
    						    LOGE("out params not implement");
    						}*/

    						//jmethodID methodID = GetMethodID(self->jniEnv, (jclass)clazz, "<init>", "()V");
    						Method *methodID = dvmFindDirectMethodByDescriptor(clazz, "<init>", "()V");
    						//jobject obj = NewObject(self->jniEnv, (jclass)clazz, methodID);
                            Object *obj = dvmClassNewInstance(clazz, methodID);
    						if (obj == NULL) {
                                char  buf[256];
    						...
    						if (IS_CLASS_FLAG_SET(clazz, CLASS_CAR_CLASS)) {
    						    dvmReleaseCARClassObject(*(u4 *)(((char *)obj)+clazz->objectSize));
    						    *(u4*)(((char *)obj)+clazz->objectSize) = pResult->i - sizeof(void*);
    						} else {  /*CLASS_CAR_INTERFACE*/
    						    *(u4*)(((char *)obj)+clazz->objectSize) = pResult->i;
    						}
 ...
 */
void dvmCallSynchronizedCARJNIMethod(const u4* args, JValue* pResult,
    const Method* method, Thread* self)
{
    Object* lockObj;
    ...
    if (obj == NULL) {
        ...
        for (i = 0; i < gDvm.globalCARClass->tableSize; i++) {
            ...
            if (pEnt->data != NULL && pEnt->data != HASH_TOMBSTONE) {
                ...
                clz = (ClassObject *)pEnt->data;
                ifxIndex = ProbeIfxByName(clz, clzName, ifxName, true);

                if (ifxIndex >= 0) {
                    u4* pCARObject = (u4*)dvmCreateCARClassObject(clz, NULL);
                    ifc = (u4*)((char*)pCARObject + sizeof(u4)*ifxIndex);
                    break;
                }
            }
        }
    } else {
        for (i = 0, clz = ((Object*)obj)->clazz;  clz != NULL;  clz = clz->super) {
            if (IS_CLASS_FLAG_SET(clz, CLASS_CAR_CLASS|CLASS_CAR_INTERFACE)) {
                ifxIndex = ProbeIfxByName(clz, clzName, ifxName, false);

                if (ifxIndex >= 0) {
                    ifc = (u4*)*(u4*)(((char*)obj) + ((Object*)obj)->clazz->objectSize + sizeof(u4)*i) + ifxIndex;
                    break;
                }
...
/*
 */
void dvmRemoveAllReference(Object *obj)
{
    Thread* self = dvmThreadSelf();
    JNIEnv* env = self->jniEnv;

    deleteLocalReference(env, (jobject)obj);
    deleteGlobalReference((jobject)obj);
}
