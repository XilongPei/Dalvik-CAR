...
static void callCARFunJavaObjMoved(const void *toObj, void *fromObj);
...
static void scavengeReference(Object **obj)
{
    ...
    callCARFunJavaObjMoved(toObj, fromObj);
#endif
...
static void scavengeBlock(HeapSource *heapSource, size_t block)
{
    ...
    while (cursor < end) {
        ...
        if (word != 0) {
            ...
            /*
             * if the object has been given up, release it.
             */
            if (isForward(cursor) != 0)
                dvmReleaseCompoundedCARObj(cursor);
...
static void callCARFunJavaObjMoved(const void *toObj, void *fromObj)
{
    ...
    for (clz = ((Object*)fromObj)->clazz;  clz->super != NULL;  clz = clz->super) {
        if (IS_CLASS_FLAG_SET(clz, CLASS_CAR_CLASS)) {
	        pCARObject = *(u4*)(((char*)fromObj) + ((Object*)fromObj)->clazz->objectSize + sizeof(u4)*layerCount);
            ...
            if (clz->pCARFunJavaObjMoved != 0) {
                if ((clz->pCARFunJavaObjMoved & IMethodInfoInU4_Mask) != 0) {
                    clz->pCARFunJavaObjMoved = (u4)dvmGetCARMethodinfoFunctionAddr((u4*)pCARObject,
                            (u4*)(clz->pCARFunJavaObjMoved & ~IMethodInfoInU4_Mask), clz->interfaceIndex);
                }
                ((JavaObjMoved)clz->pCARFunJavaObjMoved)(toObj, fromObj);
            }

            layerCount++;
        } else if (IS_CLASS_FLAG_SET(clz, CLASS_CAR_INTERFACE)) {
            pCARObject = *(u4*)(((char*)fromObj) + ((Object*)fromObj)->clazz->objectSize + sizeof(u4));
            if (pCARObject != (u4)NULL) {
                if (clz->pCARFunJavaObjMoved != 0) {
                    if ((clz->pCARFunJavaObjMoved & IMethodInfoInU4_Mask) != 0) {
                        clz->pCARFunJavaObjMoved = (u4)dvmGetCARMethodinfoFunctionAddr((u4*)pCARObject,
