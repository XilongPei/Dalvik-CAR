...
typedef enum ClassFlags {
	...
	CLASS_CAR_CLASS         = (1<<24),  // class is a CAR class
	CLASS_CAR_INTERFACE     = (1<<23),  // class is a CAR interface
	CLASS_CAR_NEEDCLEAN     = (1<<22),  // class is a CAR interface
 ...
 * Get/set class flags.
 */
#define SET_FIELD_FLAG(field, flag) \
    do { (field)->accessFlags |= (flag); } while (0)

#define CLEAR_FIELD_FLAG(field, flag) \
    do { (field)->accessFlags &= ~(flag); } while (0)

#define IS_FIELD_FLAG_SET(field, flag) \
    (((field)->accessFlags & (flag)) != 0)

#define GET_FIELD_FLAG_GROUP(field, flags) \
    ((u4)((field)->accessFlags & (flags)))

/*
 * these value's defination is for Field->accessFlags, should refer to ACC_FIELD_MASK
 *                                      12345678 */
#define FIELD_GETTER_SETTER_RESOLVED  0x10000000
...
struct StaticField {
    ...
    //if this field position changed, modify offStaticField_SGobject
	u4              pSetterGetterClassObject;

    /*
     * 0xFFFF: Delegate
     * 0xFFFE: Callback
     * 0xFFFD:
     * 0xFFFC: java mathod in setter or getter
     * 0xFFF1:
     * 0xFFF0: nonsense
     */
	u2              interfaceIndexOfSetter;
	u2              interfaceIndexOfGetter;
...
struct InstField {
    ...
    //if these 2 fields position changed, modify offInstField_setter/offInstField_getter
    u4              setter;
    u4              getter;

    /*
     * if this field is a CAR callback event, interfaceIndexOfSetter=0xFFFF
     * setter = ICallbackMethodInfo;
     *
     * 0xFFFF: Delegate
     * 0xFFFE: Callback
     * 0xFFFD:
...
struct ClassObject {
    ...
    //Elastos: inside CAR support, this is the IClassInfo which is Reflected from CARClass. For no CAR class, this will be NULL.
    u4              pClassInfo;
    u4              pConstructorInfo;
    u4              pStaticClassInfo;
    u4              pCARSingletonObject;
#ifdef WITH_COPYING_GC
    u4              pCARFunJavaObjMoved;     //JavaObjMoved
    u2              interfaceIndex;
#endif
...
//Java Object Move Before or After
typedef int (*JavaObjMoved)(const void *toObj, void *fromObj);
//pCARFunJavaObjMoved in ClassObject is IMethodInfo
#define IMethodInfoInU4_Mask 0x1


typedef struct CARMethodInfo{
	//Elastos: inside CAR support, this is the IMethodInfo which is Reflect from CARClass. For no CAR class, this will be NULL.
    u4              pMethodInfo;
	u2              interfaceIndex;    //
 ...
 * or NULL if not.
 */
Method* dvmFindMethodInListByNameForCAR(const ClassObject *clazz,
        const char *name, bool *inVirtual, int *nameConflict);
