...
bool dvmThreadObjStartup(void)
{
    ...
    gDvm.classJavaLangAppletThread =
        dvmFindSystemClassNoInit("Ljava/lang/MsgLoopThread;");

    if (gDvm.classJavaLangThread == NULL ||
        //gDvm.classJavaLangThreadGroup == NULL ||
        gDvm.classJavaLangVMThread == NULL ||
        gDvm.classJavaLangAppletThread == NULL ||
        gDvm.classJavaLangThreadGroup == NULL)
    {
    ...
    gDvm.offJavaLangThread_appletThread =
        dvmFindFieldOffset(gDvm.classJavaLangThread,
        "msgLoopThread", "Ljava/lang/MsgLoopThread;");
    ...
    if (gDvm.offJavaLangThread_vmThread < 0 ||
        ...
        gDvm.offJavaLangThread_appletThread < 0 ||
    ...
    gDvm.offJavaLangAppletThread_id =
        dvmFindFieldOffset(gDvm.classJavaLangAppletThread,
            "id", "I");
    if (gDvm.offJavaLangVMThread_thread < 0 ||
        gDvm.offJavaLangAppletThread_id < 0 ||
...
bool dvmCreateInterpThread(Object* threadObj, int reqStackSize)
{
    ...
    Object  *mainAppletThread;
    mainAppletThread = dvmGetFieldObject(threadObj, gDvm.offJavaLangThread_appletThread);
    if (mainAppletThread != NULL) {
        newThread->appletThreadId = (void*)dvmGetFieldInt(mainAppletThread, gDvm.offJavaLangAppletThread_id);
    }
    newThread->pArgumentList = 0;
...
static void* interpThreadStart(void* arg)
{
    ...
    //CAR applet main thread
    if (self->appletThreadId != NULL) {
        dvmAttachMsgLoopThread(self->appletThreadId);
    } else {
        dvmAttachMsgLoopThread(gDvm.pAppletObj);
    }
    ...
    //detach from CAR applet main thread
    dvmDetachMsgLoopThread();
